...
<script>
	...
	function fn_delete(qbd_no){
		if (confirm("삭제하시겠습니까?") == true){    //확인
			 location.href="HCAM_memberQuestionDelete.jsp?qbd_no="+qbd_no;
...
<body>
	...
	<!-- section -->
	<section>
		<form action="HCAM_memberQuestionWrite01.jsp">
			<div id="total_left">
					...
					<div id="table_head">
						<table>
							...
							<% for(int i=0; i<lists.size(); i++){ %>
									<tr>
										<td><%=lists.get(i).getQbd_ctgry() %></td>
									...
									<tr class="hidden show<%=lists.get(i).getQbd_no() %>">
										...
										<td colspan="4" class="content" id="td_mfdel"><%=lists.get(i).getQbd_content() %>
										<% if(mem_no == lists.get(i).getMem_no() && ("미답변").equals(lists.get(i).getAnsState())){%>
												<br><br>
												<a onclick="location.href='HCAM_memberQuestionUpdate01.jsp?qbd_no=<%=lists.get(i).getQbd_no()%>'">수정</a>&nbsp
									...
									<% for(int j=0; j<listsAns.size(); j++){%>
										<% if(lists.get(i).getQbd_no() == listsAns.get(j).getQbd_no()) { %>
											<tr class="hidden show<%=lists.get(i).getQbd_no() %>">
												...
												<td colspan="4" class="content"><%=listsAns.get(j).getAbd_content() %><br><br><font size="2px">판매자&nbsp<%=listsAns.get(j).getAbd_date() %></font></td>
											</tr>
										<%} %>
									<%} %>
							<%} %>
