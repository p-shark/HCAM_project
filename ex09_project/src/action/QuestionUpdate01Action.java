...
public class QuestionUpdate01Action implements Action{

	/* web작업시 예외처리: throws Exception */
	@Override
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		System.out.println("111 >>> "+request.getParameter("qbd_no"));
		int qbd_no = Integer.parseInt(request.getParameter("qbd_no"));
		
		
		QuestionService questionservice = new QuestionService();
		/* 게시글 조회  */
		QuestionBoardDTO list = questionservice.getListByQbdNo(qbd_no);
		
		HcamFileDAO fileDao = new HcamFileDAO();
		/* 게시글 파일명 조회 */
		String fileName = fileDao.getFileName("qbd", qbd_no);
		/* 파일경로 조회 */
		String filePath = fileDao.getFilePath("qbd", qbd_no);
		
		fileDao.dbClose();
		
		request.setAttribute("qbd_no", qbd_no);
